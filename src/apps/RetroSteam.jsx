...
export default function RetroSteam() {
  ...
  return <div className="game-library">
    ...
    <div className="library-scroll">{game ? <>
      ...
      <div className={`game-art game-art-large art-${game.id}`}><span className="game-art-number">{game.number}</span><span className="game-art-title">{game.title}</span><span className="art-caption">{game.category}</span></div>
      ...
      <div className="game-grid">{games.map(item => <button className="game-card" key={item.id} onClick={() => setSelected(item.id)} aria-label={`View ${item.title}`}><div className={`game-art art-${item.id}`}><span className="game-art-number">{item.number}</span><span className="game-art-title">{item.title}</span><span className="art-caption">{item.category}</span></div><div className="game-card-meta"><h2>{item.title}</h2><ArrowUpRight size={19} /></div><p>{item.desc}</p><span className="game-year">{item.year} <span>View project</span></span></button>)}</div>
